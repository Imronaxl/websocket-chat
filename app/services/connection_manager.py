...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, username: str) -> None:
        ...
        logger.info("user_connected", extra={"user_id": user_id, "username": username})
    ...
    async def disconnect(self, websocket: WebSocket) -> None:
        ...
        logger.info("user_disconnected", extra={"user_id": user_id, "username": username})
    ...
    async def cleanup_dead_connections(self) -> None:
        ...
        for ws in dead:
            ...
            logger.info("closing_dead_connection", extra={"client_host": client_host})
            await self.disconnect(ws)

    async def handle_join_room(self, websocket: WebSocket, msg: JoinRoomMessage) -> None:
        room_id = msg.room_id
        ...
        logger.debug("user_joined_room", extra={"username": username, "room_id": room_id})

    async def handle_leave_room(self, websocket: WebSocket, msg: LeaveRoomMessage) -> None:
        room_id = msg.room_id
        user_info = self.connection_info[websocket]
        user_id = user_info["user_id"]
        username = user_info["username"]
        ...
        logger.debug("user_left_room", extra={"username": username, "room_id": room_id})

    async def handle_chat_message(self, websocket: WebSocket, msg: ChatMessage) -> None:
        room_id = msg.room_id
        user_info = self.connection_info[websocket]
    ...
    async def _listen_redis(self, websocket: WebSocket) -> None:
        pubsub: Optional[PubSub] = None
        try:
            pubsub = redis_client.client.pubsub()
            await pubsub.psubscribe("room:*")

            logger.debug("redis_listener_started", extra={"client": getattr(websocket, 'client', None)})
            ...
            logger.exception("redis_listener_error", extra={"error": str(e)})
        finally:
            if pubsub:
                try:
